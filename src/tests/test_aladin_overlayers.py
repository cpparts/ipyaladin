...
def test_overlayers_dict_add_graphic_overlay_from_stcs_iterables(
    ...
    assert test_name in aladin._overlays_dict
    assert aladin._overlays_dict[test_name]["type"] == "overlay"
    regions_info = mock_send.call_args[0][0]["regions_infos"]
    assert regions_info == aladin._overlays_dict[test_name]["regions_infos"]
    ...
    assert aladin._overlays_dict[test_name + "_1"]["type"] == "overlay"
    regions_info = mock_send.call_args[0][0]["regions_infos"]
    assert regions_info == aladin._overlays_dict[test_name + "_1"]["regions_infos"]
    assert regions_info[0]["infos"]["stcs"] in stcs_strings

    # test handling for overlay with no given name
    aladin.add_graphic_overlay_from_stcs(stcs_strings)

    assert "overlay_python" in aladin._overlays_dict
    assert aladin._overlays_dict["overlay_python"]["type"] == "overlay"
    regions_info = mock_send.call_args[0][0]["regions_infos"]
    assert regions_info == aladin._overlays_dict["overlay_python"]["regions_infos"]
    assert regions_info[0]["infos"]["stcs"] in stcs_strings
...
def test_overlayers_dict_add_table(monkeypatch: Callable) -> None:
    ...
    assert test_name in aladin._overlays_dict
    assert aladin._overlays_dict[test_name]["type"] == "table"
    ...
    assert test_name + "_1" in aladin._overlays_dict
    assert aladin._overlays_dict[test_name + "_1"]["type"] == "table"
    assert aladin._overlays_dict[test_name + "_1"]["options"]["color"] == "pink"
    assert "circle_error" in aladin._overlays_dict[test_name + "_1"]["options"]
    ...
    assert "catalog_python" in aladin._overlays_dict
    assert aladin._overlays_dict["catalog_python"]["type"] == "table"
